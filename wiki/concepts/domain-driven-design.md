...
In AI-assisted development the ubiquitous language gains a second user: the agent. A common failure
mode is the human and the model "talking at cross-purposes" — the agent being verbose because it
lacks the shared terms the human takes for granted, the same translation gap that exists between a
developer and a domain expert. The fix is the same as the human one: maintain the ubiquitous
language as an explicit glossary (a markdown file of terms and meanings, often generated by scanning
the codebase) and keep it in front of the agent during planning and implementation. Practitioners
report this both tightens planning *and* makes the model's reasoning less verbose and more aligned
with the plan — the agreed terms double as steering [[agent-skill-design|leading words]] that recur
in the model's reasoning traces. This makes a documented ubiquitous language a concrete input to the
[[ai-assisted-development-workflow]], not just human-to-human hygiene.
