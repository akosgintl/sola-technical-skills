---
title: "Full Walkthrough: Workflow for AI Coding (Matt Pocock workshop)"
aliases: [pocock ai coding workshop, grill to PRD to kanban to AFK]
type: source
domain: ai-agentic
status: seed
tags: [source, transcript, ai-coding, workflow, ralph-loop, afk-agents, tdd]
updated: 2026-06-30
source_url: https://www.youtube.com/watch?v=-QFHIoCo-Ko
source_type: transcript
ingested: 2026-06-30
feeds: [ai-assisted-development-workflow, tracer-bullets, deep-modules, git-worktrees-parallel-agents, context-engineering, delegate-review-own]
---

# Full Walkthrough: Workflow for AI Coding — Matt Pocock

> [!info] Source metadata
> **Author:** Matt Pocock (aihero.dev) · **Format:** ~2h conference workshop · **URL:** https://www.youtube.com/watch?v=-QFHIoCo-Ko
> Transcript via YouTube auto-captions. Long single-session walkthrough building a gamification feature into a course platform.

## Key takeaways

- Two LLM constraints to design around: (1) a smart zone / dumb zone — context degrades as it fills (marker ~100K tokens regardless of advertised window; bigger windows just "ship more dumb zone"); (2) LLMs are like the guy from Memento — they forget on every clear. He prefers clearing over compacting because a cleared context always returns to the same clean state, whereas compaction accumulates sediment.
- End-to-end workflow: idea -> Grill Me (alignment / shared design concept) -> write a PRD (the destination document; he does not read it back, only checking the LLM summarization) -> PRD-to-issues as a Kanban board with blocking relationships (a DAG, parallelizable) -> AFK implementation (Ralph loop) -> QA + code review.
- Human-in-the-loop vs AFK tasks: planning/alignment must be human-in-the-loop (often multiple humans, mob-style); implementation can be turned into an AFK (away from keyboard) task. Framed as day shift (human plans) / night shift (agent implements).
- Tracer bullets / vertical slices (Pragmatic Programmer): AI loves to code horizontally (all DB, then all API, then all UI) which defers feedback to the last phase. Force thin vertical slices that cross every layer so feedback arrives during phase one.
- Ralph (Ralph Wiggum) loop: specify the destination (PRD), then loop "make a small change toward it." once.sh cats all issue markdown + last 5 commits + prompt into Claude with accept-edits; the AFK version loops in a Docker sandbox.
- TDD red-green-refactor is essential: writing the failing test first instruments the code before implementation, making it harder for the agent to cheat the tests. Feedback-loop quality is the ceiling on AI output quality.
- Deep vs shallow modules (Ousterhout) again: shallow-module codebases are hard for AI to navigate and hard to draw test boundaries around; deep modules give a big simple test boundary. Design the interface, delegate the implementation ("gray boxes") to retain a sense of the codebase while moving fast. Skill: improve codebase architecture.
- Review in the smart zone: clear context before the automated review step so the reviewer isn't reasoning in the dumb zone that produced the code. Uses Sonnet to implement, Opus to review.
- Push vs pull for standards: let the implementer pull coding standards (skills it may read), but push standards to the reviewer.
- Doc rot: he discards PRDs/issues after completion (closes GitHub issues) rather than keeping markdown that drifts from the code and misleads future agents.
- Sandcastle: his TypeScript library for AFK loops — creates a git worktree, sandboxes it in Docker, runs planner -> per-issue implementer sandboxes -> reviewer -> merger agent. Turns the sequential Ralph loop into parallel agents.
- QA is where the human imposes taste; fully automating idea/QA/research/prototype yields tasteless slop. Prototypes (e.g. three throwaway front-end variants) feed feedback back into grilling.

## Notable claims (with location)

- Dex Horthy (Human Layer) coined the smart-zone/dumb-zone framing; attention scales quadratically with tokens.
- "The rate of feedback is your speed limit." Bad codebases make bad agents — feedback-loop quality caps AI output quality.
- Kanban DAG enables parallelization: independent issues become "independently grabbable" by parallel agents; a sequential numbered plan can only be run by one loop.
- Out-of-scope section in the PRD captures negative decisions and definition of done.
