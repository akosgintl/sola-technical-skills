...
aliases: [loop engineering osmani, five pieces of a loop, loop anatomy]
type: source
domain: ai-agentic
status: seed
tags: [source, agent-loops, loop-engineering, skills, sub-agents, worktrees, automations]
updated: 2026-06-26
...
> **Author/Org:** Addy Osmani (Director, Google Cloud AI; author of *Beyond Vibe Coding*) · **Date:** 2026-06-07 · **URL:** https://addyosmani.com/blog/loop-engineering/

## Key takeaways

- **Definition:** "Loop engineering is replacing yourself as the person who prompts the agent. You design the system that does it instead." A loop is a *recursive goal* — define a purpose and the AI iterates until complete.
- Loop engineering sits **one floor above the [[agentic-harness|harness]]**: the harness is the environment one agent runs inside; the loop is the harness "but it runs on a timer, it spawns little helpers, and it feeds itself." It also relates to Osmani's "factory model" (the system that builds the software).
- **Not a tool thing anymore.** A year ago a loop meant a pile of bespoke bash you maintained forever. Now the five primitives ship inside the products — Steinberger's list maps almost exactly onto the Codex app *and* onto Claude Code. Design a loop that works regardless of which tool you sit in.
- **The five pieces + memory** (the canonical anatomy): automations, worktrees, skills, plugins/connectors, sub-agents — plus external memory (a markdown file or Linear board) because "the agent forgets, the repo doesn't."
- **Skills kill intent debt.** An agent starts every session cold and fills any gap in your intent with a confident guess ("intent debt"). A skill writes that intent down on the outside — conventions, build steps, the "we don't do it like this because of that one incident" — read every run. Without skills the loop re-derives the whole project from zero every cycle; with skills it compounds. (Skill = authoring format; plugin = how you ship it.)
- **Sub-agents: keep the maker away from the checker.** The model that wrote the code is too nice grading its own homework. A second agent (different instructions, sometimes a different model) catches what the first talked itself into — "adversarial code review." This is what Claude Code's `/goal` does under the hood: a fresh model decides if the loop is done.
- **Three problems get *sharper* as the loop gets better, not easier:** (1) **verification is still on you** — "a loop running unattended is also a loop making mistakes unattended"; "done" is a claim, not a proof; (2) **comprehension debt** — the faster the loop ships code you didn't write, the bigger the gap between what exists and what you understand; (3) **cognitive surrender** — the comfortable posture of taking whatever the loop returns without an opinion.
- **Same action, opposite results.** "Two people can build the exact same loop and get completely opposite results. One uses it to move faster on work they understand deeply. The other uses it to avoid understanding the work at all. The loop doesn't know the difference. You do."
- **The orchestration tax:** worktrees remove the mechanical collision of parallel agents, but *your review bandwidth* is still the ceiling on how many you can run — not the tool.

## Notable claims (with location)

- Peter Steinberger: "You shouldn't be prompting coding agents anymore. You should be designing loops that prompt your agents." **Boris Cherny**, head of Claude Code at Anthropic: "I don't prompt Claude anymore. I have loops running that prompt Claude and figuring out what to do. My job is to write loops." (intro) — *Note: this corrects the "Boris Turney" mis-transcription in the earlier YouTube source.*
- `/loop` re-runs on a cadence; `/goal` keeps going until a written condition is true, with a separate small model checking "done" after each turn. Codex has the same `/goal` (run-until-done, pause/resume/clear). (Automations section)
- Both Codex app and Claude Code now ship all five primitives; the article gives a primitive-by-primitive mapping table (Automations / Worktrees / Skills / Plugins-connectors / Sub-agents / State).
- Closing: "Build the loop. But build it like someone who intends to stay the engineer, not just the person who presses go." Cherny's point isn't that the work got easier — "the leverage point moved."
